...
- An optional linked-stack ID order is the sole durable grouping state, with element zero as the semantic parent. Link All Notes to Current Note (Cmd+Shift+L or any note's chain button) makes the invoking note the parent without moving it, resets every active member to 300px wide, and orders the other active notes by their current top-to-bottom then left-to-right positions. Children share the parent's left edge and follow the preceding active note's actual height plus a 12px gap, including collapsed 24px notes; the exact hierarchy may extend beyond the work area. Resizing, folding, and restoring notes reflows only the affected note's following siblings, while archiving the parent temporarily promotes the first active child as the visible anchor. During a macOS title-bar drag, every other open member is temporarily attached as a native child behind the active note so AppKit moves the group synchronously; other platforms apply the dragged member's delta. Unlink Notes preserves current geometry, while Reset Note Positions (Cmd+Shift+R) unlinks first and places active notes in a visible cascade without restoring archived notes.
...
- Cmd+Shift+R reopens any missing active note window, resets every active note into a visible cascade inside the primary display's work area, and persists the recovered positions without reopening archived notes.
